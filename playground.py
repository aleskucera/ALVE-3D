...
superpoint_map = np.array([0, 0, 0, 1, 1, 0, 2, 2, 2])

# Map values to superpoints
superpoints = np.unique(superpoint_map)
superpoint_values = np.zeros(len(superpoints))
for i, superpoint in enumerate(superpoints):
    superpoint_values[i] = values[superpoint_map == superpoint]
