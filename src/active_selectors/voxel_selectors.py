...
class VoxelCloud(object):
    ...
    def add_values(self, values: torch.Tensor, voxel_map: torch.Tensor) -> None:
        """ Add values to the cloud.

        :param values: The values to add to the cloud with shape (N,), where N is the number of values
        :param voxel_map: The voxel map with shape (N,), where N is the number of values. The voxel map maps
                          each value to a voxel in the cloud.
        """
    ...
    def get_std_values(self):
        """ Calculate the standard deviation of each voxel and return the values, voxel indices and cloud ids.

        :return: A tuple containing the values, voxel indices and cloud ids
        """

        # Initialize the values tensor with NaNs
        ...
        values[unique_voxels.type(torch.long)] = voxel_stds

        return self._append_mapping(values)

    def get_random_values(self):
        """ Return random values for each voxel and return the values, voxel indices and cloud ids.
    ...
    def map_model_outputs_to_clouds(self, dataset: Dataset, model: nn.Module):
        ...
        with torch.no_grad():
            ...
            for i in tqdm(range(dataset.get_length()), desc='Mapping model output values to voxels'):
                ...
                proj_image, proj_voxel_map = proj_image.to(self.device), proj_voxel_map.to(self.device)

                # Get the model output for the sample
                model_output = torch.rand((proj_image.shape[1], proj_image.shape[2], 1), device=self.device)
