...
class Sequence:
    def __init__(self, seq, path=None, poses_path=None, pose_provider='surf_slam'):
        assert isinstance(seq, (int, str))
        if isinstance(seq, str):
            parts = seq.split('/')
            assert 1 <= len(parts) <= 2
            if len(parts) == 2:
                assert parts[0] == 'semantic_kitti'
                seq = parts[1]
        if path is None:
    ...
    def __str__(self):
        return 'semantic_kitti/' + self.sequence
...
def subseq_demo():
    ...
    for i, name in enumerate(dataset_names):
        ds = Dataset(name='semantic_kitti/' + name, pose_provider='odom')
