...
def compute_weight_loss(args, embeddings, objects, edg_source, edg_target, is_transition, diff, return_partition,
                        xyz=0):
    if args.loss_weight == 'seal' or args.loss_weight == 'crosspartition' or return_partition:
        ...
        pred_components, pred_in_component = None, None

    if args.loss_weight == 'none':
        weights_loss = np.ones_like(edg_target).astype('f4')
    elif args.loss_weight == 'proportional':
        weights_loss = np.ones_like(edg_target).astype('f4') * float(len(is_transition)) / (
                1 - is_transition).sum().float()
        weights_loss[is_transition.nonzero()] = float(len(is_transition)) / float(
            is_transition.sum()) * args.transition_factor
        weights_loss = weights_loss.cpu().numpy()
    elif args.loss_weight == 'seal':
        weights_loss = compute_weights_SEAL(pred_components, pred_in_component, objects, edg_source, edg_target,
                                            is_transition, args.transition_factor)
    elif args.loss_weight == 'crosspartition':
        weights_loss = compute_weights_XPART(pred_components, pred_in_component, objects.cpu().numpy(), edg_source,
                                             edg_target, is_transition.cpu().numpy(),
                                             args.transition_factor * 2 * args.k_nn_adj, xyz)
    else:
...
def compute_weights_SEAL(pred_components, pred_in_component, objects, edg_source, edg_target, is_transition,
                         transition_factor):
    ...
    for i_com in range(len(pred_components)):
        w_per_component[i_com] = len(pred_components[i_com]) - mode(objects[pred_components[i_com]],
                                                                    only_frequency=True)
    SEAL_weights[is_transition.nonzero()] += np.stack( \
        (w_per_component[pred_in_component[edg_source[is_transition.nonzero()]]]
         , w_per_component[pred_in_component[edg_target[is_transition.nonzero()]]])).max(
        0) * transition_factor  # 1 if not transition 1+w otherwise
    return SEAL_weights


def compute_weights_XPART(pred_components, pred_in_component, objects, edg_source, edg_target, is_transition,
                          transition_factor, xyz):
