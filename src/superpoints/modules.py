        ...
        def message(self, edge_index_i, x_j, size_i, weights):
            if not self.vv:
                ...
                if self.softmax:  # APPLY A TWO DIMENSIONAL NON-DEPENDENT SPARSE SOFTMAX
                    ...
                    weight = torch.cat(
                        [softmax(weight[:, k, :], edge_index_i, size_i).unsqueeze(1) for k in range(self.out_channels)],
                        dim=1)
                ...
                if self.softmax:
                    weight = F.leaky_relu(weight, self.negative_slope)
                    weight = torch.cat([softmax(w.unsqueeze(-1), edge_index_i, size_i).t() for w in weight.t()],
                                       dim=0).t()
                return x_j * weight
...
class GRUCellEx(nn.GRUCell):
    ...
    def forward(self, input, hidden):
        ...
        if input.is_cuda and torch.__version__.split('.')[0] == '0':
            ...
            try:  # pytorch >=0.3
                return state.apply(gi, gh, hidden) if self.bias_ih is None else state.apply(gi, gh, hidden,
                                                                                            self.bias_ih, self.bias_hh)
            except:  # pytorch <=0.2
                return state()(gi, gh, hidden) if self.bias_ih is None else state()(gi, gh, hidden, self.bias_ih,
                                                                                    self.bias_hh)
...
class LSTMCellEx(nn.LSTMCell):
    ...
    def forward(self, input, hidden):
        ...
        if input.is_cuda and torch.__version__.split('.')[0] == '0':
            ...
            try:  # pytorch >=0.3
                return state.apply(gi, gh, hidden[1]) if self.bias_ih is None else state.apply(gi, gh, hidden[1],
                                                                                               self.bias_ih,
                                                                                               self.bias_hh)
            except:  # pytorch <=0.2
                return state()(gi, gh, hidden[1]) if self.bias_ih is None else state()(gi, gh, hidden[1], self.bias_ih,
                                                                                       self.bias_hh)
