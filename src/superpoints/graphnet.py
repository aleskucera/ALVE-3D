...
class GraphNetwork(nn.Module):
    ...
    def __init__(self, config, nfeat, fnet_widths, fnet_orthoinit=True, fnet_llbias=True, fnet_bnidx=-1,
        ...
        for d, conf in enumerate(config.split(',')):
            ...
            if conf[0] == 'f':  # Fully connected layer;  args: output_feats
            ...
            elif conf[0] == 'gru' or conf[
                0] == 'lstm':  # RNN-ECC     args: repeats, mv=False, layernorm=True, ingate=True, cat_all=True
                nrepeats = int(conf[1])
                vv = bool(int(conf[2])) if len(
                    conf) > 2 else True  # whether ECC does matrix-value mult or element-wise mult
                ...
                fnet = create_fnet(fnet_widths + [nfeat ** 2 if not vv else nfeat], fnet_orthoinit, fnet_llbias,
                                   fnet_bnidx)
                if conf[0] == 'gru':
