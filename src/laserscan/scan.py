...
class LaserScan:
    ...
    def set_scan(self, points: np.ndarray, remissions: np.ndarray = None, color: np.ndarray = None,
        ...
        if color is None and self.colorize:
            ...
            self.color = colorize(self.radius, color_map='turbo', data_range=rem_range)
    ...
    def set_label(self, semantics: np.ndarray, instances: np.ndarray = None) -> None:
        ...
        # Colorize label
        if self.colorize:
            self.sem_label_color = map_colors(self.sem_label, self.sem_color_map)
            self.inst_label_color = colorize_instances(self.inst_label)

            self.proj_sem_color = map_colors(self.proj_sem_label, self.sem_color_map)
            self.proj_inst_color = colorize_instances(self.proj_inst_label)
    ...
    def set_entropy(self, entropy: np.ndarray) -> None:
        ...
        if self.colorize:
            ...
            self.entropy_color = colorize(self.entropy, color_map='jet', data_range=entropy_range)
            self.proj_entropy_color = colorize(self.proj_entropy, color_map='jet', data_range=entropy_range)
    ...
    def set_superpoints(self, superpoints: np.ndarray) -> None:
        ...
        if self.colorize:
            ...
            self.superpoints_color = colorize(self.superpoints, color_map='jet', data_range=sp_range)
            self.proj_superpoints_color = colorize(self.proj_superpoints, color_map='jet', data_range=sp_range)
